...
const Courses = ({user}) => {
  ...
  return (
    ...
    {/* <input
        ...
        <label htmlFor="search" className="leading-7 text-sm text-gray-50 text-[20px]">Search Here</label>
        <input type="text"  value={query} onChange={handleSearchChange}  placeholder="Search courses..." id="search" name="search" className="w-full bg-gray-800 rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-100 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out"/>
      </div>
      <div className="flex flex-wrap -m-4">
       {filteredData.map((course)=>(
        <div key={course.id} className="p-4 md:w-1/3">
          <div className="h-full border-2 border-gray-200 border-opacity-60 rounded-lg overflow-hidden">
            <Image className="lg:h-48 md:h-36 w-full object-cover object-center" src={course.image_link} alt="blog" width={800} height={400}/>
            <div className="p-6">
